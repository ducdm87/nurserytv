...
//        echo '<br />init frontend';
    ...
    public function getVideoHots(){
        $video = new Media();
        return $video->getMedias(3,0,array('f.status=1'),array('f.viewed DESC'));
    }
    
    public function getNewVideo(){
        $video = new Media();
        return $video->getMedias(1,0,array('f.status=1'),array('f.cdate DESC'));
    }
}
